...
public class Que2 {
    ...
    public static void make_graph(int N, int[][] prerequisites, ArrayList<HashSet<Integer>> graph) {
        for (int i = 0; i < prerequisites.length; i++) {
            graph.get(prerequisites[i][1]).add(prerequisites[i][0]);
        }
    }

    static boolean dfs_cycle(ArrayList<HashSet<Integer>> graph, int node, int[] onpath, int[] visited) {
    ...
    public static boolean isPossible(int N, int[][] prerequisites) {
        ...
        for (int i = 0; i < N; i++) {
            if (vis[i] == 0 && dfs_cycle(graph, i, onpath, vis))
                return false;
        }
        return true;
